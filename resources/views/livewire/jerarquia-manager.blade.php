<div class="row g-3" wire:key="jerarquia-manager-root">
    <div class="col-md-2">
        <div class="card module-form-card border-0 h-100">
            ...
            <div class="card-body p-3 bg-white rounded-bottom">
                ...
                <div class="mb-3">
                    <label for="jerarquia-prefijo" class="form-label small text-muted fw-semibold">{{ __('Prefijo') }}</label>
                    <input type="text" id="jerarquia-prefijo" wire:model="prefijo" class="form-control form-control-sm rounded-3 @error('prefijo') is-invalid @enderror" placeholder="{{ __('Prefijo') }}">
                    @error('prefijo')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    <label for="jerarquia-organizacion" class="form-label small text-muted fw-semibold">{{ __('Organización') }}</label>
                    <input type="text" id="jerarquia-organizacion" wire:model="organizacion" class="form-control form-control-sm rounded-3 @error('organizacion') is-invalid @enderror" placeholder="{{ __('Organización') }}">
                    @error('organizacion')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                                    ...
                                    <td class="text-end pe-2">
                                        ...
                                        @else
                                            <div class="d-inline-flex gap-1 justify-content-end">
                                                ...
                                                <!-- Editar -->
                                                <button type="button" wire:click="edit({{ $jerarquia->id }})"
                                                ...
                                                <button type="button" wire:click="askDelete({{ $jerarquia->id }})"
                                                    class="btn btn-light p-0 d-flex align-items-center justify-content-center"
