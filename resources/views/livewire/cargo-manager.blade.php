<div class="row g-4" wire:key="cargo-manager-root">
    <div class="col-md-3">
        <div class="card module-form-card shadow-sm border-0 h-100">
            ...
            <div class="card-body p-3 bg-white rounded-bottom">
                ...
                <div class="mb-3">
                    <label for="cargo-nombre" class="form-label small text-muted text-uppercase fw-semibold">{{ __('Nombre') }}</label>
                    <input type="text" id="cargo-nombre" wire:model="nombre" class="form-control rounded-3 @error('nombre') is-invalid @enderror" placeholder="{{ __('Nombre') }}">
                    @error('nombre')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-4">
                    <label for="cargo-prefijo" class="form-label small text-muted text-uppercase fw-semibold">{{ __('Prefijo') }}</label>
                    <input type="text" id="cargo-prefijo" wire:model="prefijo" class="form-control rounded-3 @error('prefijo') is-invalid @enderror" placeholder="{{ __('Prefijo') }}">
                    @error('prefijo')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                                    ...
                                    <td class="text-end pe-3">
                                        @if ($confirmingDeleteId === $cargo->id)
                                            <div class="d-inline-flex flex-column align-items-end gap-1">
                                                <span class="small text-danger">{{ __('¿Eliminar este cargo?') }}</span>
                                        ...
                                        @else
                                            <div class="d-inline-flex gap-1 justify-content-end">
                                                <button type="button" wire:click="edit({{ $cargo->id }})" class="btn btn-light btn-icon border shadow-sm" title="{{ __('Editar') }}">
                                                    <i class="fa-solid fa-pen text-secondary"></i>
                                                </button>
                                                <button type="button" wire:click="askDelete({{ $cargo->id }})" class="btn btn-light btn-icon border shadow-sm" title="{{ __('Eliminar') }}">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center text-muted py-5">{{ __('No hay cargos registrados.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
