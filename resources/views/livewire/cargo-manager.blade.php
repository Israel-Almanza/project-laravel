<div class="row g-3" wire:key="cargo-manager-root">
    <div class="col-md-2">
        <div class="card module-form-card border-0 h-100">
            ...
            <div class="card-body p-3 bg-white rounded-bottom">
                ...
                <div class="mb-3">
                    <label for="cargo-nombre" class="form-label small text-muted fw-semibold">{{ __('Nombre') }}</label>
                    <input type="text" id="cargo-nombre" wire:model="nombre" class="form-control form-control-sm rounded-3 @error('nombre') is-invalid @enderror" placeholder="{{ __('Nombre') }}">
                    @error('nombre')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    <label for="cargo-prefijo" class="form-label small text-muted fw-semibold">{{ __('Prefijo') }}</label>
                    <input type="text" id="cargo-prefijo" wire:model="prefijo" class="form-control form-control-sm rounded-3 @error('prefijo') is-invalid @enderror" placeholder="{{ __('Prefijo') }}">
                    @error('prefijo')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                                    ...
                                    <td class="text-end pe-2">
                                        ...
                                        @else
                                            <div class="d-inline-flex gap-1 justify-content-end">
                                                ...
                                                <!-- Editar -->
                                                <button type="button" wire:click="edit({{ $cargo->id }})"
                                                ...
                                                <button type="button" wire:click="askDelete({{ $cargo->id }})"
                                                    class="btn btn-light p-0 d-flex align-items-center justify-content-center"
