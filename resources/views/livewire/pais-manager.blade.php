<div class="row g-3" wire:key="pais-manager-root">
    <div class="col-md-2">
        <div class="card module-form-card border-0 h-100">
            ...
            <div class="card-body p-3 bg-white rounded-bottom">
                ...
                <div class="mb-3">
                    <label for="pais-prefijo"
                        class="form-label small text-muted fw-semibold">{{ __('Prefijo') }}</label>
                    <input type="text" id="pais-prefijo" samll wire:model="prefijo"
                        class="form-control form-control-sm rounded-3 @error('prefijo') is-invalid @enderror"
                        placeholder="{{ __('ARG') }}">
                    @error('prefijo')
                ...
                <div class="mb-3">
                    <label for="pais-nombre" class="form-label small text-muted  fw-semibold">{{ __('Pais') }}</label>
                    <input type="text" id="pais-nombre" wire:model="nombre"
                        class="form-control form-control-sm rounded-3 @error('nombre') is-invalid @enderror"
                        placeholder="{{ __('Argentina') }}">
                    @error('nombre')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                <div class="row">
                    <div class="col-md-8 mb-3">
                        <label for="pais-coordenadas"
                            class="form-label small text-muted  fw-semibold">{{ __('Coordenadas') }}</label>
                        <input type="text" id="pais-coordenadas" wire:model="coordenadas"
                            class="form-control form-control-sm rounded-3 @error('coordenadas') is-invalid @enderror"
                            placeholder="{{ __('-33.44683, -70.66138') }}">
                        @error('coordenadas')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-4">
                        ...
                        <input type="text" id="pais-zoom" wire:model="zoom"
                            class="form-control form-control-sm rounded-3 @error('zoom') is-invalid @enderror"
                            placeholder="{{ __('15') }}">
                        @error('zoom')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                ...
                @if ($editingId)
                    <button type="button" wire:click="cancel"
                        class="btn btn-outline-secondary w-100 mt-2 rounded-3">{{ __('Cancelar') }}</button>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="col-md-10">
        <div class="card module-table-card shadow-sm border-0">
            <div
                ...
                <span class="fw-semibold mb-0">{{ __('Pais') }}</span>
                                    ...
                                    <td class="text-end pe-2">
                                        ...
                                        @else
                                            <div class="d-inline-flex gap-1 justify-content-end">
                                                ...
                                                <!-- Editar -->
                                                <button type="button" wire:click="edit({{ $pai->id }})"
                                                ...
                                                <button type="button" wire:click="askDelete({{ $pai->id }})"
                                                    class="btn btn-light p-0 d-flex align-items-center justify-content-center"
